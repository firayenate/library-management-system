...
if ($method === 'GET') {
    $stmt = $conn->prepare("SELECT student_id as id, full_name as name, email, phone, department, year_of_study as year, profile_picture, created_at FROM users WHERE student_id = ?");
    ...
    if(isset($data['action']) && $data['action'] === 'updateProfile') {
        ...
        $stmt->bind_param("ssssss", $name, $email, $phone, $dept, $year, $student_id);
        if($stmt->execute()) {
